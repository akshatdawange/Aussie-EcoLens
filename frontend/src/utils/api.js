...
export const normalizeMedia = (item = {}) => {
  const thumbnailUrl =
    item.thumbnailUrl || item.thumbnail_url || item.thumbUrl ||
    item.thumb_url || item.thumbnail || null;
  const originalUrl =
    item.originalUrl || item.original_url || item.fullUrl || item.full_url ||
    item.url || item.imageUrl || item.image_url || item.s3Url || null;
  const fileId =
  ...
  const fileType =
    item.fileType || item.file_type || item.type ||
    (originalUrl && /\.(mp4|mov|webm|avi)$/i.test(originalUrl) ? 'video' : 'image');
...
export const uploadFile = async (file) => {
  ...
  if (presignRes.status === 401) throw new Error('Not logged in or token expired');
...
const ownerIdOf = (f) =>
  f.ownerSub || f.userId || f.ownerId || f.owner || f.userSub || f.cognitoSub ||
  f.uploadedBy || f.uploaderId || f.sub || null;
